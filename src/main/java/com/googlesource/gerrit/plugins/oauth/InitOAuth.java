...
class InitOAuth implements InitStep {
  ...
  @Inject
  InitOAuth(ConsoleUI ui, Section.Factory sections,
      @PluginName String pluginName) {
    this.ui = ui;
    this.googleOAuthProviderSection = sections.get(PLUGIN_SECTION,
        pluginName + GoogleOAuthService.CONFIG_SUFFIX);
    this.githubOAuthProviderSection = sections.get(PLUGIN_SECTION,
        pluginName + GitHubOAuthService.CONFIG_SUFFIX);
    this.bitbucketOAuthProviderSection = sections.get(PLUGIN_SECTION,
  ...
  public void run() throws Exception {
    ...
    if (configureGoogleOAuthProvider) {
      ...
      googleOAuthProviderSection.string("Link to OpenID accounts?",
          LINK_TO_EXISTING_OPENID_ACCOUNT, "true");
    }

    boolean configureGitHubOAuthProvider =
